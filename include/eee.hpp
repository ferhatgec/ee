...
namespace e {
    static std::unordered_map<std::string, std::string> __e__{
            {"E100",  "Curcumin"},
    ...
    static inline std::string get(std::string __e) noexcept {
        std::transform(__e.begin(), __e.end(), __e.begin(),
            [](unsigned char ch) { return std::toupper(ch); });

        auto data = __e__.find(__e);

        if(data != __e__.end()) { return data->second; }
